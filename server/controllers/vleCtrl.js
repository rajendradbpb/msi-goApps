...
exports.exportVle = function(req, res) {
  ...
  //   distinct = 'urbanType';
  // }
  console.log("exports.exportVle query   ", req.query);
  console.log("exports.exportVle params   ", params);
  models.vleModel.find(params).populate("district block gp").exec()
    .then(function(data) {
      var dataArr = [];
      for (var i in data) {
...
exports.exportVleSummary = function(req, res) {
  var exportProperties = [];
  var params = {
    //isDelete: false,
  };
  if(req.query.covertype == "block"){
    params['block'] = { "$ne": null }
  }
  else if(req.query.covertype == "gp"){
    params['urban'] = false;
  }
  else if(req.query.covertype == "urban"){
    params['urban'] = true;
  }
  if (req.query.exportProperties) {
    exportProperties = req.query.exportProperties.split(",");
  }
  console.log("exports.exportVleSummary query ",req.query);
  console.log("exports.exportVleSummary params ",params);
  models.vleModel.find(params).populate("district block gp").exec()
    .then(function(data) {
      ...
      for (var i in data) {
        ...
        if (exportProperties.indexOf("dist") != -1)
          obj.dist = data[i].dist ? data[i].dist : "NA";
        if (exportProperties.indexOf("state") != -1)
          obj.state = data[i].state ? data[i].state : "Odisha";
        if (exportProperties.indexOf("country") != -1)
          obj.country = "India";
        if (exportProperties.indexOf("pin") != -1)
          obj.pin = data[i].pin ? data[i].pin : "NA";
